    ...
    open func applyTransform(to attributes: FSPagerViewLayoutAttributes) {
        ...
        case .overlap,.linear:
            ...
            let scale = max(1 - (1-self.minimumScale) * abs(position), self.minimumScale)
                let transform = CGAffineTransform(scaleX: scale, y: 1).translatedBy(x: 0, y: 0)
                attributes.size.height = attributes.size.height * scale
                attributes.frame.origin.y += attributes.frame.origin.y * scale
                print("newPosition: \(position)")
                print("scale: \(scale)")
            attributes.transform = transform
//                attributes.size = CGSize(width: attributes.size.width, height: attributes.size.height * scale)
//                attributes.frame = CGRect(x: attributes.frame.origin.x, y: attributes.frame.origin.y * scale, width: attributes.size.width, height: attributes.size.height * scale)
